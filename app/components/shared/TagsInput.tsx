...
const TagsInput = ({label, tags, setTags, disabled = false}: Props) => {
    ...
    return (
        <div className="tags-input-container">
            <div className="ks-wt-form-input-container-wrapper ks_d_flex ks_flex_col">
                {label ? <label>Cc</label>: null}
                <ul className="ks-form-tag-it-list ks_flex_no_wrap ks_scrollable">
                    {
                        tags.map((tag, index) => (
                            <li key={index} className="ks-form-tag-it-list-item">
                                <label>{tag}</label>
                                <CustomTooltip placement={"top"} title={"remove"}>
                                    <svg onClick={() => removeTag(index)} viewBox="0 0 22 22">
                                        <path
                    ...
                    <li className="ks-form-tag-it-list-item-new">
                        <ClickOutside active={true} onClick={() => {
                            if(ref.current) {
                            ...
                            <input ref={ref} disabled={disabled} onKeyDown={handleKeyDown} type="text" placeholder={disabled ? '' : 'Cc by hit space to add more'} />
                        </ClickOutside>
                    </li>
                </ul>
            </div>
