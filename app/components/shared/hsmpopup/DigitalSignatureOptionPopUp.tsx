"use client"
import { useDgStore, useDigitalOptionStore } from "@/app/lib/store";
import { CERTIFICATE_TYPE } from "@/utils/enum";
import cn from "clsx";
import { useState } from "react";
import { Modal } from "react-bootstrap";
import SelectDigitalSignature from "./SelectDigitalSignature";

interface Props {
    handleUSBComfirm: () => void;
    isLoading?: boolean;
  }

const DigitalSignatureOptionPopUp: React.FC<Props> = ({handleUSBComfirm, isLoading}) => {
    const [selectedOption, setSelectedOption] = useState('1'); // ['1', '2']

    const dsOptionStore = useDigitalOptionStore();

    const handleIssue = () => {
        if(selectedOption == CERTIFICATE_TYPE.HSM) {
            dsOptionStore?.setShowCertList(true)
            dsOptionStore?.setOpen(false)
        }else{
            handleUSBComfirm()
            dsOptionStore?.setOpen(false)
        }
    ...
    return (
        <>
                {dsOptionStore.showCertList && <SelectDigitalSignature
                    open={dsOptionStore.showCertList}
                    setOpen={dsOptionStore?.setShowCertList}
                /> }

                <Modal show={dsOptionStore?.open} dialogClassName={"modal-dialog modal-dialog-centered ks-wt-modal-xs-dialog"} style={{zIndex:"9999999999"}}>
                    <div className="modal-content ks-wt-modal-content">
                        ...
                        <div className="ks-wt-modal-footer d-flex justify-content-end align-items-center">
                            <div className="ks-wt-element-group-container d-flex align-items-center">
                                <button type={"button"} onClick={handleCancel} className="ks_btn ks_btn_tiary">Cancel</button>
                                <button
                                    type={"button"}
                                    className={cn("ks_btn ks_btn_pm ", { "ks-wt-disabled": isLoading })}
                                    onClick={handleIssue}
                                >
...
export default DigitalSignatureOptionPopUp;
