...
export function getSignBy(xml: string) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(xml, "application/xml");

    // Access elements and attributes using the DOM tree
    const noteElement = doc.documentElement;
    const dn = noteElement.querySelector('X509SubjectName')?.textContent || ''
...
//check if already has percentage at the end of string if not add it
export const formatAddPercentage = (value: string) => {
